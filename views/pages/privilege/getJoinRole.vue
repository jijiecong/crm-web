...
<script type="text/javascript">
  ...
  export default{
    ...
    methods: {
      ...
      get_table_data(){
        this.load_data = true
        this.$http.get(request_uri.getJoinRoleList, {
          params: {
      ...
      delete_data(item){
        ...
        this.$msgbox({
          ...
          type: 'warning',
          message: h('p', null, [
            h('span', null, '此操作将删除该角色和当前权限的所有关联, 是否继续?'),
          ]),
          ...
          let param = this.$qs.stringify({privilegeId: this.route_id, roleId: item.roleId})
          this.$http.post(request_uri.delRoleHasPrivilege, param)
      ...
      delete_all(){
        ...
        this.$msgbox({
          ...
          type: 'warning',
          message: h('p', null, [
            h('span', null, '此操作将删除当前权限和所有角色的关联, 是否继续?'),
          ]),
          showCancelButton: true,
        }).then((action) => {
          if (action === 'cancel') {
