      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="props">
            <span v-text="statusText(props.row.hasRoleStatus)"></span>
          </template>
        </el-table-column>
        <el-table-column
          label="操作"
    ...
    methods: {
      ...
      delete_data(item){
        ...
        this.$msgbox({
          ...
          type: 'warning',
          message: h('p', null, [
            h('span', null, '此操作将删除该用户和当前角色的所有关联, 是否继续?'),
          ]),
          ...
          let param = this.$qs.stringify({roleId: this.route_id, userId: item.userId})
          this.$http.post(request_uri.delUserHasRole, param)
      ...
      delete_all(){
        ...
        this.$msgbox({
          ...
          type: 'warning',
          message: h('p', null, [
            h('span', null, '此操作将删除当前角色和所有用户的关联, 是否继续?'),
          ]),
          showCancelButton: true,
        }).then((action) => {
          if (action === 'cancel') {
